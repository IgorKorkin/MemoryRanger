...
_Use_decl_annotations_ void EptHandleEptViolation(
  ...
  if (!exit_qualification.fields.ept_readable &&
    ...
    if (!ept_entry || !ept_entry->all) {
      ...
      EptpConstructTables(processor_data->ept_data_default_enclave->ept_pml4, 4,
        fault_pa, processor_data->ept_data_default_enclave, true);

      RweConstructTablesForEnclaves(EptpConstructTables, 4, fault_pa, true);
