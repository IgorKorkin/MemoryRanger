...
_Use_decl_annotations_ static void VmpInitializeVm(
  ...
  // Set up EPT
  processor_data->ept_data_default = EptInitialization();
  processor_data->ept_data = processor_data->ept_data_default;
  if (!processor_data->ept_data_default) {
    goto ReturnFalse;
  }
...
//  vm_procctl_requested.fields.cr8_load_exiting = false;  // NB: very frequent
  ...
  error |= UtilVmWrite(VmcsField::kCr0ReadShadow, cr0_shadow.all  /*__readcr0()*/ );
  error |= UtilVmWrite(VmcsField::kCr4ReadShadow, cr4_shadow.all  /*__readcr4()*/ );
...
_Use_decl_annotations_ static void VmpFreeProcessorData(
  ...
  if (processor_data->ept_data_default) {
    EptTermination(processor_data->ept_data_default);
  }
  if (processor_data->ept_data_protected_driver) {
    EptTermination(processor_data->ept_data_protected_driver);
  }
  if (processor_data->rwe_data) {
