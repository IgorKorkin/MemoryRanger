...
_When_(ept_data == nullptr,
    ...
    *EptpConstructTables(_In_ EptCommonEntry *table, _In_ ULONG table_level,
                         ...
                         _In_opt_ bool default_access = true);
...
_Use_decl_annotations_ EptData *EptInitialization(bool default_access) {
  PAGED_CODE();
  ...
  ept_poiner->fields.memory_type =
	  static_cast<ULONG64>(EptpGetMemoryType(UtilPaFromVa(ept_pml4))); // static_cast<ULONG64>(memory_type::kWriteBack);
  ...
  for (auto run_index = 0ul; run_index < pm_ranges->number_of_runs;
    ...
    for (auto page_index = 0ull; page_index < run->page_count; ++page_index) {
      ...
      const auto ept_pt_entry =
          EptpConstructTables(ept_pml4, 4, indexed_addr, nullptr, default_access);
      if (!ept_pt_entry) {
  ...
  switch (table_level) {
    case 4: {
      ...
      if (!ept_pml4_entry->all) {
        ...
        EptpInitTableEntry(ept_pml4_entry, table_level, UtilPaFromVa(ept_pdpt), default_access);
      }
      return EptpConstructTables(
          reinterpret_cast<EptCommonEntry *>(
              UtilVaFromPfn(ept_pml4_entry->fields.physial_address)),
          table_level - 1, physical_address, ept_data, default_access);
    }
    case 3: {
      // table == PDPT (1 GB)
      ...
      if (!ept_pdpt_entry->all) {
        ...
        EptpInitTableEntry(ept_pdpt_entry, table_level, UtilPaFromVa(ept_pdt), default_access);
      }
      return EptpConstructTables(
          reinterpret_cast<EptCommonEntry *>(
    ...
    case 2: {
      ...
      if (!ept_pdt_entry->all) {
        ...
        EptpInitTableEntry(ept_pdt_entry, table_level, UtilPaFromVa(ept_pt), default_access);
      }
      return EptpConstructTables(
          reinterpret_cast<EptCommonEntry *>(
    ...
    case 1: {
      ...
      EptpInitTableEntry(ept_pt_entry, table_level, physical_address, default_access);
      return ept_pt_entry;
    }
    default:
...
_Use_decl_annotations_ static void EptpInitTableEntry(
    EptCommonEntry *entry, ULONG table_level, ULONG64 physical_address, bool default_access) {
  entry->fields.read_access = default_access;
  entry->fields.write_access = default_access;
  entry->fields.execute_access = default_access;
  entry->fields.physial_address = UtilPfnFromPa(physical_address);
  if (table_level == 1) {
    entry->fields.memory_type = 
		static_cast<ULONG64>(EptpGetMemoryType(physical_address)); // static_cast<ULONG64>(memory_type::kWriteBack);
  }
}
...
_Use_decl_annotations_ void EptHandleEptViolationShared(
...
_Use_decl_annotations_ void EptHandleEptViolation(
  ...
  if (!exit_qualification.fields.ept_readable &&
    ...
    if (!ept_entry || !ept_entry->all) {
      ...
      EptpConstructTables(processor_data->ept_data_default->ept_pml4, 4,
                          fault_pa, processor_data->ept_data_default, true);
	  
	  RweConstructTables(EptpConstructTables, 4, fault_pa, true);

// 	  if (processor_data->ept_data_protected_driver){
// 		  EptpConstructTables(processor_data->ept_data_protected_driver->ept_pml4, 4,
// 			  fault_pa, processor_data->ept_data_protected_driver, true);
// 	  }
