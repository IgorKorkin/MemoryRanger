...
_Use_decl_annotations_ static void VmmpHandleMsrAccess(
  ...
  // Do not shadow 64bit fields because the current implementation for x86 is not
...
_Use_decl_annotations_ static void VmmpHandleGdtrOrIdtrAccess(
  ...
  auto operation_address = segment_base /**/ + base_value + index_value + displacement;
  ...
  // memory.
  const auto guest_cr3 = VmmpGetKernelCr3();//  UtilVmRead(VmcsField::kGuestCr3);
    ...
    case MovCrAccessType::kMoveToCr:
      switch (exit_qualification.fields.control_register) {
        ...
        case 0: {
          ...
          UtilVmWrite(VmcsField::kGuestCr0, cr0.all /* *register_used*/);
          UtilVmWrite(VmcsField::kCr0ReadShadow, cr0.all/* *register_used*/);
          break;
        }
        ...
        case 3: {
		  ...
		  UtilVmWrite(VmcsField::kGuestCr3, (*register_used & ~(1ULL << 63))); //UtilVmWrite(VmcsField::kGuestCr3, *register_used);
          break;
        }
        ...
        case 4: {
          ...
          UtilVmWrite(VmcsField::kGuestCr4, cr4.all /* *register_used*/);
          UtilVmWrite(VmcsField::kCr4ReadShadow, cr4.all /* *register_used*/);
          break;
        }
  ...
  switch (hypercall_number) {
	...
	case HypercallNumber::kShEnablePageShadowing:
		ShEnablePageShadowing(
			guest_context->stack->processor_data->ept_data_default,
			guest_context->stack->processor_data->shared_data->shared_sh_data);
	...
	case HypercallNumber::kShDisablePageShadowing:
		ShVmCallDisablePageShadowing(
			...
			guest_context->stack->processor_data->shared_data->shared_sh_data);
		ShVmCallDisablePageShadowing(
			guest_context->stack->processor_data->ept_data_protected_driver,
			guest_context->stack->processor_data->shared_data->shared_sh_data);
		
		VmmpIndicateSuccessfulVmcall(guest_context);
		break;
	//////////////////////////////////////////////////////////////////////////
	case HypercallNumber::kMemRangerSetEptNewDriver:
		if (context) {
			//guest_context->stack->processor_data->ept_data_protected_driver
			ShEnablePageShadowing(
				(EptData*)context,
		...
		break;
// 		if (!guest_context->stack->processor_data->ept_data_protected_driver){
// 			guest_context->stack->processor_data->ept_data_protected_driver = (EptData*)context;
// 			ShEnablePageShadowing(
// 				guest_context->stack->processor_data->ept_data_protected_driver,
// 				guest_context->stack->processor_data->shared_data->shared_sh_data);
// 			VmmpIndicateSuccessfulVmcall(guest_context);
