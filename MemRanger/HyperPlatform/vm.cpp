...
_Use_decl_annotations_ static SharedProcessorData *VmpInitializeSharedData() {
  ...
  shared_data->shared_sh_data = ShAllocateSharedShadowHookData();
...
_Use_decl_annotations_ static void VmpInitializeVm(
  ...
  // Set up EPT
  processor_data->ept_data_default_enclave = EptInitialization();
  processor_data->ept_data = processor_data->ept_data_default_enclave;
  if (!processor_data->ept_data_default_enclave) {
      VmpFreeProcessorData(processor_data);
      return;
  }

  /*token*/processor_data->ept_data_token = EptInitialization();
  /*token*/RweSetDefaultEptAttributesForEpt(processor_data->ept_data_token);
...
_Use_decl_annotations_ static void VmpFreeProcessorData(
  ...
  if (processor_data->ept_data_default_enclave) {
      EptTermination(processor_data->ept_data_default_enclave);
  }
  /*token*/if (processor_data->ept_data_token) {
      EptTermination(processor_data->ept_data_token);
  }
