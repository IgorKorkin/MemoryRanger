...
_Use_decl_annotations_ static void VmmpHandleMsrAccess(
  ...
  if (read_access) {
    if (transfer_to_vmcs) {
      ...
      //
      // else if ( ){}
      // https://github.com/tandasat/HyperPlatform/commit/19d657ea38d454a08348948adf180b042e16051a
      //////////////////////////////////////////////////////////////////////////
...
_Use_decl_annotations_ static void VmmpHandleVmCall(
  ...
  switch (hypercall_number) {
    ...
    case HypercallNumber::kShEnablePageShadowing:
      ShEnablePageShadowingForNewEnclave(
        guest_context->stack->processor_data->ept_data_default_enclave,
        guest_context->stack->processor_data->shared_data->shared_sh_data);

      /*token*/ShEnablePageShadowingForNewEnclave(
          guest_context->stack->processor_data->ept_data_token,
          guest_context->stack->processor_data->shared_data->shared_sh_data);

//       ShEnablePageShadowing(
//         guest_context->stack->processor_data->ept_data_monitor,
//         guest_context->stack->processor_data->shared_data->shared_sh_data);
      VmmpIndicateSuccessfulVmcall(guest_context);
      break;
    ...
    case HypercallNumber::kShDisablePageShadowing:
      ...
      /*token*/ShVmCallDisablePageShadowing(
          guest_context->stack->processor_data->ept_data_token,
          guest_context->stack->processor_data->shared_data->shared_sh_data);
...
//     GuestContext *guest_context) {
//   // Clear the TS flag temporally since XSAVE/XRSTOR raise #NM
//   Cr0 cr0 = {__readcr0()};
