...
namespace payloads{
	...
	byte* retrive_func_addr_from_jmp(const PAYLOAD_FUNC payloadFuncAsJmpAddr) {
		...
		byte* jmp_trampoline = (byte*)payloadFuncAsJmpAddr;
		if ((jmp_trampoline[0] == 0xE9) && (jmp_trampoline[5] == 0xE9)) {
			func_addr = jmp_trampoline +
				jmp_trampoline[1] +
...
// 		}
		...
		if (!_pidOffset) {
			for (int i = 0; i < 0x64; i++) {
				if ((g_bDefaultPid[0] == funcAddr[i]) &&
					(g_bDefaultPid[1] == funcAddr[i + 1]) &&
					(g_bDefaultPid[2] == funcAddr[i + 2]) &&
					(g_bDefaultPid[3] == funcAddr[i + 3])) {
					_pidOffset = i;
					break;
				}
			}
		...
		if (_pidOffset && targetPid) {
			__try {
				funcAddr[_pidOffset] = (byte)(targetPid & 0x000000FF);
				funcAddr[_pidOffset + 1] = (byte)((targetPid & 0x0000FF00) >> 8);
				funcAddr[_pidOffset + 2] = (byte)((targetPid & 0x00FF0000) >> 8 * 2);
				funcAddr[_pidOffset + 3] = (byte)((targetPid & 0xFF000000) >> 8 * 3);
	...
	bool set_memory_permission(const DWORD flNewProtect, byte *funcBegin) {
		...
		if (funcBegin) {
			...
			for (unsigned char *p = NULL;
				VirtualQueryEx(handle_process, p, &info, sizeof(info)) == sizeof(info);
				p += info.RegionSize) {
				if (((size_t)info.BaseAddress <= (size_t)funcBegin) &&
					((size_t)funcBegin <= ((size_t)info.BaseAddress + info.RegionSize))) {
					DWORD old_protect = 0;
					b_res =
						(0 != VirtualProtectEx(handle_process,
							info.BaseAddress, info.RegionSize, flNewProtect, &old_protect));
